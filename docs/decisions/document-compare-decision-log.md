...
| D11 | Render `SideBySideDiff` insertions **green** and deletions **red** — the same colour signal the inline redline view uses — with WCAG-2-AA-compliant Tailwind shades (`text-green-700`/`bg-green-50` ≈ 4.79:1; `text-red-700`/`bg-red-50` ≈ 5.91:1), keeping the non-color `underline` / `line-through` cues; legend dots use `bg-red-600` / `bg-green-600` to echo the inline `#dc2626` / `#16a34a`. | (a) Keep side-by-side insertions brand-azure (`text-blue-700`) and merely document the green-inline-vs-azure-sidebyside divergence. (b) Repaint the inline view azure by editing `globals.css .docx-view-container ins`. (c) Use the inline's exact green-600 `#16a34a` for side-by-side. | A single feature showing insertions in two hues — green inline, azure side-by-side — reads as two different signals (QA FIN-C Issue 1, MAJOR). The inline colours live in `globals.css .docx-view-container`, which is **shared** with the reference-only, must-remain-untouched `DocxView.tsx`, so alternative (b) is out of scope — the side-by-side view is reconciled toward the inline green instead. Alternative (c) fails WCAG 2 AA: green-600 `#16a34a` measures only 3.16–3.30:1 on the panel background (QA Issue 3, MINOR), so the darker green-700 / red-700 **text** shades are used to reach ≥ 4.5:1 while staying the same colour family; the retained non-color cues keep WCAG 1.4.1 (use of colour) passing. | Side-by-side text is one step darker (700) than the inline redline's 600, so the two views share a colour *signal* but not byte-identical hex.<br>Accepted: exact-hex parity is impossible without either failing AA or editing the shared inline CSS; the red-600 / green-600 legend dots bridge the two views and both spans exceed 4.5:1. |
...
| R18 | Inline redline + side-by-side diff rendering (tokenized, accessible, responsive) | `frontend/src/app/components/compare/RedlineDocxView.tsx` (inline via `docx-preview`)<br>`frontend/src/app/components/compare/SideBySideDiff.tsx` (two-column + change nav) | Green (inserted) / red (deleted) matching the inline redline, WCAG-AA-compliant shades (D11); muted/border tokens, `Button` nav, `aria-pressed`, small-screen stacking; no `TextSearchWidget` (deviation (c)). |
